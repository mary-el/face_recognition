...
from utils import set_headers, open_doors, read_db, read_excel, get_encodings, print_log, previous_state


def show_recognized_faces(frame, face_locations, recognized_ids, reduce_frame, left_area, right_area):
    cv2.rectangle(frame, (left_area[0] * reduce_frame, left_area[1] * reduce_frame), (left_area[2] * reduce_frame, left_area[3] * reduce_frame), (0, 255, 255), 2)
    cv2.rectangle(frame, (right_area[0] * reduce_frame, right_area[1] * reduce_frame), (right_area[2] * reduce_frame, right_area[3] * reduce_frame), (255, 0, 0), 2)
...
def video_capture(id_to_encoding, camera, dict_users, reduce_frame=2, show=True, test=False):
    ...
    left_area = [int(area_1[0] * width), int(area_1[1] * height), int((area_1[0] + area_1[2]) * width), int((area_1[1] + area_1[3]) * height)]
    right_area = [int(area_2[0] * width), int(area_2[1] * height), int((area_2[0] + area_2[2]) * width), int((area_2[1] + area_2[3]) * height)]
    stop = False
    while not stop:
        ret, frame = cap.read()
...
if __name__ == '__main__':
    ...
    print_log(f'started at {datetime.now().strftime("%D:%H:%M:%S")} mode= {config["mode"]} mode_paranoid={mode_paranoid }')
